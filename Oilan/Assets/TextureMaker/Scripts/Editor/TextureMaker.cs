...
namespace TextureMaker
{
    ...
    public class TextureMaker
    {
        ...
        public static Texture2D FillPattern(int width, int height, Vector2Int count, Color c1, Color c2, float turnFraction, bool flip = false)
        {
            Texture2D tex = new Texture2D(width, height, TextureFormat.RGBA32, true);
            tex.name = "tex_pattern_.png";
            ...
            //    colors[(int)Mathf.Abs((x * width)) + (int)Mathf.Abs((y * height)) * width] = c1;
            //}
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    float angle = 2 * Mathf.PI * turnFraction * x * y;
                    float xx = dX * Mathf.Cos(angle);
                    float yy = dY * Mathf.Sin(angle);


                    float root = Mathf.Sqrt(xx * xx + yy * yy);

                    colors[x + y * tex.width] = Color.Lerp(c1, c2, yy / maxCount);
                }
            }

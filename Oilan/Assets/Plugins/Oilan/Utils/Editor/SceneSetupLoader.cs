...
namespace Oilan
{
    ...
    public class SceneSetupLoader : MonoBehaviour
    {
        ...
        {
            data = (SceneSetupData)AssetDatabase.LoadAssetAtPath("Assets/_Oilan/_Scenes/SceneSetups/SceneSetup.asset", typeof(SceneSetupData)); //+"Assets/Data/SceneSetup.asset"
        }
        ...
        static void Load(int index)
        {
            if (data == null)
            {
                Debug.LogError("Scene setup data not configured. Plase add it to 'Assets/_Oilan/_Scenes/SceneSetups' folder");
            }
            else
            {
                ...
                {
                    Debug.LogError(string.Format("Scene setup with index {0} not configured. Plase add it to 'Assets/_Oilan/_Scenes/SceneSetups' folder", index));
                }
                else
                {
